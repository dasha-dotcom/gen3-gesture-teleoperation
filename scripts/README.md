...
|---|---|---|
| `mac/hand_sender.py` | Mac | Normal camera-to-UDP palm sender |
...
| `ubuntu/gen3_hand_bridge_v3.py` | Ubuntu | Calibrated hold-to-run y/z mapping into Servo |
...
Keep the Servo launch and YAML together. Use only one command source at a time; a disabled bridge still publishes zeros. The current hand bridge has no two-second hold cap.
...
The tests load only the pure `Gate` and mapping definitions from the bridge, without starting its ROS node, socket, or GUI. They cover calibrated symmetry, the diagonal speed cap, hand loss, stale messages, invalid coordinates, and explicit re-enabling. They do not validate ROS, focus/key events, camera access, network timing, or physical stopping latency.
