...
def classify(points):
    ...
    for base in (5, 9, 13, 17):
        ...
        if (
            ...
            bend < 115
            and distance(mcp, tip) < 1.6 * length
        ):
...
def main():
    ...
    parser.add_argument(
        "--send",
        action="store_true",
        help="Send palm and gesture data to Ubuntu",
    )
    ...
    try:
        if not camera.isOpened():
            raise RuntimeError(
                "Cannot open camera. "
                "Close other camera previews first."
            )
        ...
        with hands_api.Hands(
            ...
            while True:
                ...
                if result.multi_hand_landmarks:
                    ...
                    u = (
                        sum(p.x for p in palm)
                        / len(palm)
                    )

                    v = (
                        sum(p.y for p in palm)
                        / len(palm)
                    )

                    # Apparent palm width in image pixels.
                    # Landmarks 5 and 17 are the MCP joints
                    # at opposite sides of the palm.
                ...
                if key == ord("p"):
                    print(
                        f"\nGesture: {label}; raw: {raw}",
                        flush=True,
                    )

                    print(
                        "OPEN requires middle > 140 "
                        "and tip > 140 for all "
                        "four fingers.",
                        flush=True,
                    )
